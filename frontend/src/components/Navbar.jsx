          ...
          {currentUser ? (
            <Link to="/profile">
              <img 
                src={profileIcon} 
                alt="Profile" 
                className="w-8 h-8 p-0.5 rounded-full cursor-pointer border-2 border-main" 
              />
            </Link>
          ) : (
            <Link to="/signup">
              <button className="bg-main px-2 py-1 text-white font-[600] rounded-full text-[14px] hover:opacity-90 phoneMax:px-3 phoneMax:py-2">
                Sign Up
              </button>
            </Link>
          )}
